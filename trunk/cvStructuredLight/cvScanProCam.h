...
#ifndef CV_SCAN_PRO_CAM_H_
#define CV_SCAN_PRO_CAM_H_
...
public:
    CVScanProCam(CVCamera* camera_);
    ~CVScanProCam();

    // Run the background capture (used to eliminate background points from reconstructions).
    int runBackgroundCapture(struct slParams* sl_params, struct slCalib* sl_calib);

    // Run the structured light scanner.
    int runStructuredLight(struct slParams* sl_params, struct slCalib* sl_calib, int scan_index);
