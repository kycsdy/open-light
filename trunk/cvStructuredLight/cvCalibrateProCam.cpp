...
// Constructor
CVCalibrateProCam::CVCalibrateProCam(CVCamera *camera_)
{
    camera = camera_;
}

// Destructor
CVCalibrateProCam::~CVCalibrateProCam()
{

}

// Display the camera calibration results to the console.
void CVCalibrateProCam::displayCamCalib(struct slCalib* sl_calib){
...
void CVCalibrateProCam::displayProjCalib(struct slCalib* sl_calib){
...
int CVCalibrateProCam::generateChessboard(struct slParams* sl_params, IplImage*& board, int& border_cols, int& border_rows){
...
int CVCalibrateProCam::detectChessboard(IplImage* frame, CvSize board_size,
...
int CVCalibrateProCam::runCameraCalibration(struct slParams* sl_params, 
    ...
    sl_params->cam_w = frame->width;
    sl_params->cam_h = frame->height;
...
int CVCalibrateProCam::runProjectorCalibration(struct slParams* sl_params, 
					        struct slCalib* sl_calib,
							bool calibrate_both){
	...
	while(successes < n_boards)
    {
        ...
        cam_frame = camera->QueryFrameSafe();
		...
		cvDrawChessboardCorners(cam_frame, cam_board_size, cam_corners, cam_corner_count, cam_found);
		...
		if(cam_corner_count == cam_board_n){
			...
			if(cvKey_temp != -1) 
				cvKey = cvKey_temp;
		    // Get next available "safe" frame.
            cam_frame_1 = camera->QueryFrameSafe();
			//cvCvtColor(cam_frame_1, cam_frame_1_gray, CV_RGB2GRAY);
            cvSplit(cam_frame_1, NULL, cam_frame_1_gray, NULL, NULL);
            cvShowImageResampled("camWindow", cam_frame_1_gray, sl_params->window_w, sl_params->window_h);
			...
			cvDrawChessboardCorners(cam_frame_2, proj_board_size, proj_corners, proj_corner_count, proj_found);
			cvShowImageResampled("camWindow", cam_frame_2, sl_params->window_w, sl_params->window_h);
...
//// Run projector-camera calibration (including intrinsic and extrinsic parameters).
//int CVCalibrateProCam::runProjectorCalibrationOrig(struct slParams* sl_params, 
//					        struct slCalib* sl_calib,
//							bool calibrate_both){
//
//	// Reset projector (and camera) calibration status (will be set again, if successful.
...
int CVCalibrateProCam::runProCamExtrinsicCalibration(struct slParams* sl_params, 
					              struct slCalib* sl_calib){
...
int CVCalibrateProCam::evaluateProCamGeometry(struct slParams* sl_params, struct slCalib* sl_calib){
