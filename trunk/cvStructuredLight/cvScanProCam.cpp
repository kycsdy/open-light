...
CVScanProCam::CVScanProCam(CVCamera *camera_)
{
    camera = camera_;
}

CVScanProCam::~CVScanProCam()
{

}

// Generate Gray codes.
int CVScanProCam::generateGrayCodes(int width, int height, 
					  IplImage**& gray_codes, 
...
// Decode Gray codes.
int CVScanProCam::decodeGrayCodes(int proj_width, int proj_height,
					IplImage**& gray_codes, 
...
int CVScanProCam::slScan(IplImage**& proj_codes, IplImage**& cam_codes,
		   int n_cols, int n_rows, 
...
int CVScanProCam::displayDecodingResults(IplImage*& decoded_cols, 
						   IplImage*& decoded_rows, 
						   IplImage*& mask,
						   struct slParams* sl_params){
...
// Display a depth map.
int CVScanProCam::displayDepthMap(CvMat*& depth_map,
					IplImage*& mask,
				    struct slParams* sl_params){
...
int CVScanProCam::reconstructStructuredLight(struct slParams* sl_params, 
					           struct slCalib* sl_calib,
...
int CVScanProCam::runBackgroundCapture(struct slParams* sl_params, 
						 struct slCalib* sl_calib){
	...
	camPreview(camera, sl_params, sl_calib);
	...
	slScan(proj_gray_codes, cam_gray_codes, gray_ncols, gray_nrows, sl_params, sl_calib);
...
int CVScanProCam::runStructuredLight(struct slParams* sl_params, 
					   struct slCalib* sl_calib, 
					   int scan_index){
	...
	printf("Displaying the structured light sequence...\n");
	IplImage** cam_gray_codes = NULL;
	slScan(proj_gray_codes, cam_gray_codes, 
	...
	printf("Saving the point cloud (wrl)...\n");
	...
	// Save the point cloud.
	printf("Saving the point cloud (obj)...\n");
	sprintf(str, "%s\\%s\\%s_%0.2d.obj", sl_params->outdir, sl_params->object, sl_params->object, scan_index);
	if(savePointsOBJ(str, points, NULL, colors, mask)){
		printf("Scanning was not successful and must be repeated!\n");
		return -1;
	}
