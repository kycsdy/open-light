...
#ifndef CV_CALIBRATE_PRO_CAM_H_
#define CV_CALIBRATE_PRO_CAM_H_

class CVCalibrateProCam
{
private:
    CVCamera* camera;

    // Run projector-camera calibration (including intrinsic and extrinsic parameters).
    int runProjectorCalibrationOrig(struct slParams* sl_params, struct slCalib* sl_calib, bool calibrate_both);

public:
    CVCalibrateProCam(CVCamera *camera_);
    ...
    int runCameraCalibration(struct slParams* sl_params, struct slCalib* sl_calib);
    ...
    int runProCamExtrinsicCalibration(struct slParams* sl_params, struct slCalib* sl_calib);
